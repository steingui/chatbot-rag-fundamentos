...
- [x] **[PERF-03] [DONE]** Paralelizou recuperação Pinecone + busca web DDGS no stream via [`_fetch_sources_parallel()`](../backend/rag/chat.py:429), sobrepondo o I/O dominante para reduzir a 1ª resposta (~58s → alvo <10s). Teste de regressão em [`tests/test_stream_parallel_fetch.py`](../tests/test_stream_parallel_fetch.py:1). *(issue #14, finding 1 — HIGH)*
